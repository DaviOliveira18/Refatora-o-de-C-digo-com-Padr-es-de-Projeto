...
// Singleton para gerenciar o modal
...
// Factory Method para criar linhas da tabela
...
// Observer para notificar quando itens são carregados
class Observable {
    constructor() {
        this.observers = [];
    }
...
const itemObservable = new Observable();
const itemObserver = new ItemObserver();
itemObservable.subscribe(itemObserver);

const modalInstance = new ModalSingleton();
...
addNew.onclick = () => modalInstance.openModal();

async function editItem(id) {
    modalInstance.openModal(true, id);
}
...
async function loadItens() {
    ...
    itens.forEach(item => {
        tbody.appendChild(TableRowFactory.createRow(item));
    });

    itemObservable.notify(itens);
}
